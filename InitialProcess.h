...
public:
    ...
    void init_questioners() {
        ...
        curl = curl_easy_init();
        if (curl) {
            curl_easy_setopt(curl, CURLOPT_URL, "localhost:8080/questioners");
            curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, write_data);
            curl_easy_setopt(curl, CURLOPT_WRITEDATA, &out);
            res = curl_easy_perform(curl);
            if (res != CURLE_OK)
            ...
            ptree pt;
            read_json(out, pt);
            if (pt.count("questioners")) {
                ptree pchild = pt.get_child("questioners");
                for (auto pos = pchild.begin(); pos != pchild.end(); ++pos) {
    ...
    void init_wordbank() {
        ...
        curl = curl_easy_init();
        if (curl) {
            curl_easy_setopt(curl, CURLOPT_URL, "localhost:8080/words");
            curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, write_data);
            curl_easy_setopt(curl, CURLOPT_WRITEDATA, &out);
            res = curl_easy_perform(curl);
            if (res != CURLE_OK)
